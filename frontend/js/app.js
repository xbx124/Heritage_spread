...
function loadProjects() {
  ...
  API.get('/api/categories?size=100').then(r => {
    if (r.code === 200) document.getElementById('proj-category-filter').innerHTML = '<option value="">全部分类</option>' + r.data.list.map(c => `<option value="${c.categoryId}">${c.categoryName}</option>`).join('');
  });
...
function saveProject(e) {
  ...
  const d = { projectName: document.getElementById('proj-name').value.trim(), category_id: parseInt(document.getElementById('proj-category').value), area: document.getElementById('proj-area').value.trim(), projectIntro: document.getElementById('proj-intro').value.trim() };
...
function loadInheritors() {
  ...
  API.get('/api/projects?size=50').then(pr => {
    if (pr.code === 200) document.getElementById('inh-project-filter').innerHTML = '<option value="">全部项目</option>' + (pr.data.list || []).map(p => `<option value="${p.projectId}">${p.projectName}</option>`).join('');
  });
  API.get(url).then(res => {
    if (res.code !== 200) return;
    const d = res.data;
...
function loadNewsDetail(id) {
  API.get('/api/news/' + id).then(res => {
    ...
    document.getElementById('news-detail-content').innerHTML = `<div class="news-detail"><div class="news-title">${n.title}</div><div class="news-meta"><span>${n.author}</span><span>${n.publishTime}</span>${can ? `<span><a href="javascript:editNews(${n.newsId})" style="color:var(--cinnabar);cursor:pointer">编辑</a><a href="javascript:deleteNews(${n.newsId})" style="color:var(--cinnabar);cursor:pointer;margin-left:8px">删除</a></span>` : ''}</div><div class="news-body">${n.content || ''}</div></div>`;
