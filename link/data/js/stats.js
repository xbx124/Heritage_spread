...
async function loadProjectData() {
    ...
    if (response.code !== 200) {
        console.error('项目统计数据加载失败');
        return false;
    }
    // 后端返回 PageResult 格式 data.list（明细），按项目聚合
    const raw = response.data && response.data.list ? response.data.list : [];
    if (Array.isArray(raw) && raw.length > 0) {
        ...
        StatsState.projectData = Object.values(map);
    } else {
        StatsState.projectData = [];
    }
    renderProjectCharts();
    updateSummary();
    return true;
...
function renderProjectCharts() {
    ...
    if (!Array.isArray(data) || data.length === 0) {
        document.getElementById('chart-project').innerHTML = '<div class="loading" style="height:300px;display:flex;align-items:center;justify-content:center;color:var(--text-muted)">暂无项目数据</div>';
        document.getElementById('chart-project-bar').innerHTML = '<div class="loading" style="height:300px;display:flex;align-items:center;justify-content:center;color:var(--text-muted)">暂无项目数据</div>';
        return;
    }
